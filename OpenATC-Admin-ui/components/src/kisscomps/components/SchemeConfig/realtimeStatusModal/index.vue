...
<template>
<div>
  <div class="cross-mess" style="margin-bottom: 18px;">{{$t('openatccomponents.overview.crossinfo')}}</div>
  <div class="cross-module">
    ...
    <div class="cross-content">
      ...
      <div
        ...
        class="cross-value"
      >{{$t('openatccomponents.overview.connected')}}</div>
      <div
        v-show="devStatus===2"
        style="margin-left: 85px;"
        class="cross-value"
      >{{$t('openatccomponents.overview.offline')}}</div>
      <div
        v-show="devStatus===1"
        style="margin-left: 85px;"
        class="cross-value"
    ...
    <div class="cross-content">
      <div style="float: left;" class="cross-name">{{$t('openatccomponents.overview.signalID')}}:</div>
      <div style="margin-left: 85px;" class="cross-value">{{agentId}}</div>
    </div>
    <div class="cross-content">
      <div style="float: left;" class="cross-name">{{$t('openatccomponents.overview.signalIP')}}:</div>
  ...
  <div class="cross-module">
        ...
        <el-row :gutter="7">
          <el-col :span="12">
            <div class="grid-content bg-base">
              ...
              <div class="name">{{$t('openatccomponents.overview.controlmodel')}}</div>
            </div>
          </el-col>
          <el-col :span="12">
            <div class="grid-content bg-base">
              <div class="value">{{currModel > -1 ? $t('openatccomponents.overview.modelList' + currModel) : ''}}</div>
              <div class="name">{{$t('openatccomponents.overview.curModel')}}</div>
            </div>
          </el-col>
        </el-row>
      </div>
    ...
    <div class="cross-content" v-show="closePhase && closePhase.length > 0">
      <div
        ...
        class="cross-name"
      >{{$t('openatccomponents.overview.phaseclose')}}:</div>
      <el-tag
        type="danger"
        size="small"
        v-for="(phase, index) in closePhase"
        :key="index"
      >{{phase.typename + $t('openatccomponents.overview.phase') + phase.id + $t('openatccomponents.overview.close')}}</el-tag>
    </div>
  </div>
</div>
</template>
