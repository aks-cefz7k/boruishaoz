...
<template>
  <div class="cross-abnormal" :class="{'cross-abnormal-en': getLang() !== 'zh'}">
    <div class="abnormal-item" :class="{'odd-item': index % 2 === 0}" v-for="(faultitem, index) in mainFaultList" :key="faultitem.id">
      <div class="markIcon" :class="faultitem.imgClass"></div>
  ...
  data () {
    return {
      ...
      mainFaultList: [{
        ...
        key: 2,
        name: this.$t('openatc.faultrecord.lightcontrolversion'),
        imgClass: 'interveneImg',
        crossnum: 0
      }, {
        key: 3,
