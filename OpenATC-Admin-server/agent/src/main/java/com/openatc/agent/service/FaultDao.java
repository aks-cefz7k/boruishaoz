...
public interface FaultDao extends JpaRepository<Fault, Long> {
    ...
    @Query(value = "select agentid, m_w_faultid, m_by_fault_board_type,m_by_fault_desc_value,m_by_fault_level,m_un_fault_occur_time,m_un_fault_renew_time,m_w_fault_type,m_w_sub_fault_type,operator,enumerate,operation_time,model,event_type from fault where agentid = ?1 and m_w_faultid = ?2 and delete_flag <> '1'", nativeQuery = true)
    List<Fault> findByAgentidAndM_wFaultID(String agentid, Long m_wFaultID);

    @Query(value = "select agentid, m_w_faultid, m_by_fault_board_type,m_by_fault_desc_value,m_by_fault_level,m_un_fault_occur_time,m_un_fault_renew_time,m_w_fault_type,m_w_sub_fault_type,operator,enumerate,operation_time,model,event_type from fault where m_un_fault_renew_time > 0 and delete_flag <> '1'", nativeQuery = true)
    List<Fault> selectHistoryFaults();

    @Query(value = "select agentid, m_w_faultid, m_by_fault_board_type,m_by_fault_desc_value,m_by_fault_level,m_un_fault_occur_time,m_un_fault_renew_time,m_w_fault_type,m_w_sub_fault_type,operator,enumerate,operation_time,model,event_type from fault where delete_flag <> '1'", nativeQuery = true)
    Page<Fault> selectCurrentFaults(Specification<Fault> specification, Pageable pageRequest);

    @Query(value = "select agentid, m_w_faultid, m_by_fault_board_type,m_by_fault_desc_value,m_by_fault_level,m_un_fault_occur_time,m_un_fault_renew_time,m_w_fault_type,m_w_sub_fault_type,operator,enumerate,operation_time,model,event_type from fault where m_un_fault_renew_time = 0 and agentid=?1 and delete_flag <> '1'", nativeQuery = true)
