...
public interface FaultDao extends JpaRepository<Fault, Long> {
    ...
    @Query(value = "select id, agentid, m_w_faultid, m_by_fault_board_type,m_by_fault_desc_value,m_by_fault_level,m_un_fault_occur_time,m_un_fault_renew_time,m_w_fault_type,m_w_sub_fault_type from fault where agentid=?1 and m_un_fault_renew_time between ?4 and ?5 offset ?2 limit ?3", nativeQuery = true)
    List<Fault> selectFaultsByAgentIdAndTime(String agentId, int pageNum, int pageSize, long timeBegin, long timeEnd);
